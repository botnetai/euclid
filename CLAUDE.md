# Euclid – Dev Notes for Agents

This file provides guidance for coding agents working in this repo.

## Project Overview

Euclid is a macOS menu bar application for on‑device voice‑to‑text. It supports Whisper (Core ML via WhisperKit) and Parakeet TDT v3 (Core ML via FluidAudio). Users activate transcription with hotkeys; text can be auto‑pasted into the active app.

## Build & Development Commands

```bash
# Build the app
xcodebuild -scheme Euclid -configuration Release

# Run tests (must be run from EuclidCore directory for unit tests)
cd EuclidCore && swift test

# Or run all tests via Xcode
xcodebuild test -scheme Euclid

# Open in Xcode (recommended for development)
open Euclid.xcodeproj
```
...
2. **Model Management**: Models are managed by `ModelDownloadFeature`. Curated defaults live in `Euclid/Resources/Data/models.json`. The Settings UI shows a compact opinionated list (Parakeet + three Whisper sizes). No dropdowns.
...
5. **Permissions**: Requires audio input and automation entitlements (see `Euclid.entitlements`)

6. **Logging**: All diagnostics should use the unified logging helper `EuclidLog` (`EuclidCore/Sources/EuclidCore/Logging.swift`). Pick an existing category (e.g., `.transcription`, `.recording`, `.settings`) or add a new case so Console predicates stay consistent. Avoid `print` and prefer privacy annotations (`, privacy: .private`) for anything potentially sensitive like transcript text or file paths.
...
- WhisperKit models
  - `~/Library/Application Support/com.jjeremycai.Euclid/models/argmaxinc/whisperkit-coreml/<model>`
- Parakeet (FluidAudio)
  - We set `XDG_CACHE_HOME` on launch so Parakeet caches under the app container:
  - `~/Library/Containers/com.jjeremycai.Euclid/Data/Library/Application Support/FluidAudio/Models/parakeet-tdt-0.6b-v3-coreml`
...
- FluidAudio: `https://github.com/FluidInference/FluidAudio.git` (link `FluidAudio` to Euclid target)
...
```
XDG_CACHE_HOME = ~/Library/Containers/com.jjeremycai.Euclid/Data/Library/Application Support/com.jjeremycai.Euclid/cache
```
...
- Parakeet not detected: ensure it resides under the container path above; downloading from Euclid places it correctly.
...
2. **Use non-interactive changeset creation:** AI agents should use the non-interactive script:
   ...
   ```
3. **Only create changesets, don't process them:** Agents should only create changeset fragments. The release tool is responsible for running `changeset version` to collect changesets into `CHANGELOG.md` and syncing to `Euclid/Resources/changelog.md`.
...
3. Syncs changelog to `Euclid/Resources/changelog.md`
