...
actor SoundEffectsClientLive {
  private let logger = EuclidLog.sound
  private let baselineVolume = EuclidSettings.baseSoundEffectsVolume
  ...
  func play(_ soundEffect: SoundEffect) {
    guard euclidSettings.soundEffectsEnabled else { return }
    ...
    let clampedVolume = min(max(euclidSettings.soundEffectsVolume, 0), baselineVolume)
  ...
  func setEnabled(_: Bool) async {
    ...
    if euclidSettings.soundEffectsEnabled {
      prepareEngineIfNeeded()
    } else {
      stopAll()
      stopEngineIfNeeded()
