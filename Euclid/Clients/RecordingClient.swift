...
private let recordingLogger = EuclidLog.recording
private let mediaLogger = EuclidLog.media
...
actor RecordingClientLive {
  ...
  private func handleCaptureEnvironmentChange(reason: String, forceRestart: Bool) async {
    ...
    if euclidSettings.superFastModeEnabled {
  ...
  private func resolvePreferredInputDevice() -> AudioDeviceID? {
    if let selectedDeviceIDString = euclidSettings.selectedMicrophoneID,
  ...
  private func currentCaptureMode() -> CaptureRecordingMode {
    euclidSettings.superFastModeEnabled ? .superFast : .standard
  }
  ...
  private func makeCaptureRecordingURL() -> URL {
    FileManager.default.temporaryDirectory.appendingPathComponent("euclid-capture-\(UUID().uuidString).wav")
  }

  private func makeIgnoredStopURL() -> URL {
    FileManager.default.temporaryDirectory.appendingPathComponent("euclid-ignored-stop-\(UUID().uuidString).wav")
  }
  ...
  private func ensureInputDeviceUnmuted() {
    ...
    if let selectedIDString = euclidSettings.selectedMicrophoneID,
  ...
  func startRecording() async {
    ...
    switch euclidSettings.recordingAudioBehavior {
    case .pauseMedia:
  ...
  private func duplicateCurrentRecording() throws -> URL {
    ...
    let exportURL = recordingURL
      ...
      .appendingPathComponent("euclid-recording-\(UUID().uuidString).wav")
