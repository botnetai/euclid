...
struct MicrophoneSelectionSectionView: View {
	...
	private var selectedDeviceTag: Binding<String> {
		Binding(
			get: { store.euclidSettings.selectedMicrophoneID ?? Self.systemDefaultTag },
			set: { newValue in
	...
	private var missingSelectedDeviceID: String? {
		guard let selectedID = store.euclidSettings.selectedMicrophoneID,
	...
	private var pickerIdentity: String {
		[
			...
			store.euclidSettings.selectedMicrophoneID ?? Self.systemDefaultTag,
