# Euclid Release Pipeline - GitHub Actions Plan
...
- run: bun run tools/release.ts --bucket euclid-updates
```
...
yield* runCommandCheck("ditto", "-c", "-k", "--keepParent",
  appBundle, join(updatesDir, `Euclid-${newVersion}.zip`))
```
  ...
  release:
    ...
    steps:
      ...
      - name: Create GitHub Release
        ...
        run: |
          VERSION=$(cat Euclid/Info.plist | grep -A1 CFBundleShortVersionString | tail -1 | sed 's/.*<string>\(.*\)<\/string>.*/\1/')
          ...
          gh release create "v$VERSION" \
            --title "Euclid v$VERSION" \
            --notes-file build/release-notes-$VERSION.md \
            updates/Euclid-$VERSION.dmg \
            updates/Euclid-$VERSION.zip
```
...
# 3. Generate for "Euclid GitHub Actions"
...
```bash
# 1. Create cask
brew create --cask euclid

# 2. Test
brew install --cask euclid

# 3. Submit PR to homebrew/cask
# OR create personal tap: homebrew-euclid
```
...
bun run tools/release.ts --bucket euclid-updates-test
...
- Release tool enforces pending changesets, runs `changeset version`, and copies the aggregated `CHANGELOG.md` into `Euclid/Resources/changelog.md`
