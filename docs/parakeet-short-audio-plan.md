...
- Euclid often emits <250 ms clips when users tap-to-talk, so we must “pack” (pad) the PCM before `AsrManager.transcribe(_:)` is called.
...
2. Keep padding transparent to users: no audible artifacts, no extra latency, and logs that explain when/why padding ran (`EuclidLog.parakeet`).
...
1. `Euclid/Clients/RecordingClient.swift:667` sets up `AVAudioRecorder` at 16 kHz mono float and writes to `recording.wav` in `FileManager.default.temporaryDirectory`.
2. `Euclid/Features/Transcription/TranscriptionFeature.swift:320` calls `recording.stopRecording()` and passes the resulting URL into `TranscriptionClient.transcribe`.
3. `Euclid/Clients/TranscriptionClient.swift:242` routes Parakeet variants straight to `ParakeetClient.transcribe(_:)` with no preprocessing.
4. `Euclid/Clients/ParakeetClient.swift:118` hands the URL to `AsrManager.transcribe(url)` and returns the text.
...
2. Insert the inspector inside `TranscriptionClient` right before the Parakeet branch so we can log current clip lengths (`EuclidLog.parakeet.debug("clip=0.18s")`).
...
1. Create a new utility in `Euclid/Audio/ShortClipPadder.swift` (or similar) that:
...
2. Unit test this helper in `EuclidCoreTests` with fixtures (<50 ms clip, >threshold clip) to guarantee correct math and metadata.
3. Make the minimum configurable (env var or `EuclidSettings`) so we can tweak without hard-coding; default to whatever FluidAudio recommends.
